...
// Arc version — for parallel simulation builders
impl<T: LoopDecomposition, E: EnergyModel, P: ShiftPolicy>
    TryFrom<(Arc<NucleotideVec>, Arc<T>, Arc<E>, P)> for LoopNeighbors<E, P>
{
    type Error = String;

    fn try_from(
        (sequence, pairings, model, policy): (Arc<NucleotideVec>, Arc<T>, Arc<E>, P),
...
// Owned version — for single-shot / non-parallel use
impl<T: LoopDecomposition, E: EnergyModel, P: ShiftPolicy>
...
// Owned PairTable version — for checkpoint continuation
impl<E: EnergyModel, P: ShiftPolicy>
    TryFrom<(Arc<NucleotideVec>, PairTable, Arc<E>, P)> for LoopNeighbors<E, P>
{
    type Error = String;

    fn try_from(
        (sequence, pairings, model, policy): (Arc<NucleotideVec>, PairTable, Arc<E>, P),
    ) -> Result<Self, Self::Error> {
        let ltab = LoopTable::try_from((sequence, pairings, model))?;
        Ok(LoopNeighbors::from((ltab, policy)))
    }

...
impl<T: LoopDecomposition, E: EnergyModel> TryFrom<(Arc<NucleotideVec>, Arc<T>, Arc<E>)>
    for LoopTable<E>
{
    type Error = String;

    fn try_from(
        (sequence, pairings, model): (Arc<NucleotideVec>, Arc<T>, Arc<E>),
        ...
        pairings.for_each_loop(|l| {
            ...
            // Dereference the Arc to pass &NucleotideVec
            let loop_energy = model
                .energy_of_loop(&sequence, l)
                .expect("Broken energy evaluation!");
            energy += loop_energy;
        ...
        Ok(LoopTable {
            sequence,   // Arc<NucleotideVec> stored directly — no clone needed
            model,
            loops,
            stale: Vec::new(),
            loop_lookup,
...
impl<E: EnergyModel> TryFrom<(Arc<NucleotideVec>, PairTable, Arc<E>)>
    for LoopTable<E>
{
    type Error = String;

    fn try_from(
        (sequence, pairings, model): (Arc<NucleotideVec>, PairTable, Arc<E>),
    ) -> Result<Self, Self::Error> {
        let mut loops = Vec::new();
        let mut loop_lookup = Vec::with_capacity(sequence.len());
        let mut pair_lookup: IntMap<NAIDX, NAIDX> = IntMap::default();
